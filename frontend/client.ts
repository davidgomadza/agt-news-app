 ...
 */
import { create as api_article_create_create } from "~backend/article/create";
import { deleteArticle as api_article_delete_deleteArticle } from "~backend/article/delete";
import { get as api_article_get_get } from "~backend/article/get";
import { list as api_article_list_list } from "~backend/article/list";
import { update as api_article_update_update } from "~backend/article/update";
...
export namespace article {
    ...
    export class ServiceClient {
         ...
         */
        public async create(params: RequestType<typeof api_article_create_create>): Promise<ResponseType<typeof api_article_create_create>> {
            // Now make the actual call to the API
            const resp = await this.baseClient.callTypedAPI(`/articles`, {method: "POST", body: JSON.stringify(params)})
            return JSON.parse(await resp.text(), dateReviver) as ResponseType<typeof api_article_create_create>
        }
         ...
         */
        public async get(params: { slug: string }): Promise<ResponseType<typeof api_article_get_get>> {
            // Now make the actual call to the API
            const resp = await this.baseClient.callTypedAPI(`/articles/${encodeURIComponent(params.slug)}`, {method: "GET", body: undefined})
            return JSON.parse(await resp.text(), dateReviver) as ResponseType<typeof api_article_get_get>
        }

        /**
         * Retrieves articles, optionally filtered by category or breaking/featured status.
         */
        public async list(params: RequestType<typeof api_article_list_list>): Promise<ResponseType<typeof api_article_list_list>> {
            ...
            return JSON.parse(await resp.text(), dateReviver) as ResponseType<typeof api_article_list_list>
        }

        /**
         * Updates an existing article.
         */
        public async update(params: RequestType<typeof api_article_update_update>): Promise<ResponseType<typeof api_article_update_update>> {
            ...
            // Now make the actual call to the API
            const resp = await this.baseClient.callTypedAPI(`/articles/${encodeURIComponent(params.id)}`, {method: "PATCH", body: JSON.stringify(body)})
            return JSON.parse(await resp.text(), dateReviver) as ResponseType<typeof api_article_update_update>
        }
    }
}
